/*
   ...
   BWTBlast.c		BWT-BLAST - Local Alignment by BWT-index
...
typedef struct QueryInput {
	...
	int SeedLength;
	int SeedSkip;
	unsigned int MatchBitVector[MAX_APPROX_MATCH_ERROR];
	int match;
	int mismatch;
	int gapOpen;
	int gapExt;
	double ungapXdropoff;
	double gapXdropoff;
	double gapXdropoffFinal;
	double expectationValue;
} QueryInput;

#define SERVER_SOCKET_NAME	"BWTBLAST_SOCKET"
...
int TempErrorPatternOrder(const void *tempErrorPattern, const int index1, const int index2);

#define BWTBLAST_MAX_QUERY_LENGTH	268435456	// 256M
#define SMALL_QUERY_ALLOCATION_UNIT	65536
#define LARGE_QUERY_ALLOCATION_UNIT	1048576
#define SAINDEXGROUP_MEMORY_FACTOR	100			// 1/100 of working memory allocated for SA index group
...
#define NON_INPUT_VALUE		99999999
	...
	int WorkingMemorySize = 134217728;	// 128M - good for 16M hit; configurable through ini
										// Should never be set to < 64M
	...
	extern STAT_scoreBlock masked_stat_scoreBlock;
	extern STAT_Xdropoffs stat_XdropoffsInBits;
...
int main(int argc, char** argv) {
	...
	unsigned char *hitQueryPattern;		// For hit generation
	unsigned int *packedQueryPattern;
	...
	HitListWithPosQuery *hitList;
	GappedHitListWithAlignment* __restrict finalHitListAllContext;
	int *dbScore;
	int *dbOrder;
	int *dbRank;
	int previousMatchScore, previousMismatchScore;
	int numOfSaIndexGroupProcessed;
	int hitInCurrentRound, saIndexGroupInCurrentRound;
	int maxNumberOfSaIndexGroup;
	int queryLengthProcessed, queryLengthInCurrentRound;

	BWTSaRetrievalStatistics bwtSaRetrievalStatistics;
	HSPUngappedExtLookupTable *ungappedExtLookupTable;
	int numOfSaIndexGroup;
	SaIndexGroupNew *saIndexGroup;

	int numOfGappedHit, numOfNonCrossBoundaryGappedHit, numOfUniqueGappedHit;
...
/*
	// Error pattern
	int k;
	unsigned int *tempErrorPattern;
	int *numOfErrorPosition;
	int numOfTempErrorPattern;
	int *tempErrorPosition;
	int errorIndex;
	unsigned int errorPattern;
	int tempErrorPatternGenerated;
	HitCombination hitCombination;
	unsigned int mask;
*/
	...
	if (UnloadIndex) {
		if (Confirmation) {
			printf("Unloading BWT-BLAST server process. Press Y to go or N to cancel. ");
			c = (char)getchar();
		...
		if (SocketSend(bwtServerSocket, &QueryInput, sizeof(QueryInput)) < 0) {
			fprintf(stderr, "BWTBlast(): Socket communication error!\n");
			exit(1);
		}
	...
	if (LoadIndex) {
		...
		if (Confirmation) {
			printf("Loading BWT-BLAST server process. Press Y to go or N to cancel. ");
			c = (char)getchar();
			while (c != 'y' && c != 'Y' && c != 'n' && c!= 'N') {
				c = (char)getchar();
			}
	...
	if (!LoadIndex) {
		...
		if (bwtServerSocket != NULL) {
			...
			QueryInput.SoftMasking = SoftMasking;
			QueryInput.DustLevel = DustLevel;
			QueryInput.DustWindow = DustWindow;
			QueryInput.SeedMaxError = SeedMaxError;
			QueryInput.SeedLength = SeedLength;
			QueryInput.SeedSkip = SeedSkip;
			memcpy(QueryInput.MatchBitVector, MatchBitVector, sizeof(unsigned int) * MAX_APPROX_MATCH_ERROR);
			...
			QueryInput.ungapXdropoff = stat_XdropoffsInBits.ungapXdropoff;
			QueryInput.gapXdropoff = stat_XdropoffsInBits.gapXdropoff;
			QueryInput.gapXdropoffFinal = stat_XdropoffsInBits.gapXdropoffFinal;
	...
	// Prepare lookup table for ungapped extension
	ungappedExtLookupTable = HSPGenUngappedExtLookupTable(mmPool, stat_scoreBlock.match, stat_scoreBlock.mismatch);
	previousMatchScore = stat_scoreBlock.match;
	previousMismatchScore = stat_scoreBlock.mismatch;
	...
	for (;;) {	// Loop until unload request is received if this is a server process
		...
		if (LoadIndex) {
			...
			if (QueryInput.DatabaseName[0] == '\0') {
				...
				printf("BWT-BLAST server process is unloaded.\n");
			...
			SoftMasking = QueryInput.SoftMasking;
			DustLevel = QueryInput.DustLevel;
			DustWindow = QueryInput.DustWindow;
			SeedMaxError = QueryInput.SeedMaxError;
			SeedLength = QueryInput.SeedLength;
			SeedSkip = QueryInput.SeedSkip;
			memcpy(MatchBitVector, QueryInput.MatchBitVector, sizeof(unsigned int) * MAX_APPROX_MATCH_ERROR);
			...
			stat_XdropoffsInBits.ungapXdropoff = QueryInput.ungapXdropoff;
			stat_XdropoffsInBits.gapXdropoff = QueryInput.gapXdropoff;
			stat_XdropoffsInBits.gapXdropoffFinal = QueryInput.gapXdropoffFinal;
		...
		if (stat_scoreBlock.match != previousMatchScore || stat_scoreBlock.mismatch != previousMismatchScore) {
			HSPCalUngappedExtLookupTable(ungappedExtLookupTable, stat_scoreBlock.match, stat_scoreBlock.mismatch);
			previousMatchScore = stat_scoreBlock.match;
			previousMismatchScore = stat_scoreBlock.mismatch;
		}
...
/*
		if (SeedMaxError > 0) {
			...
			// Process MatchBitVector and generate error pattern
			numOfErrorPosition = MMUnitAllocate(SeedMaxError * sizeof(int));
			for (i=0; i<SeedMaxError; i++) {
				numOfErrorPosition[i] = 1;
				for (j=0; j<SeedLength; j++) {
					if (MatchBitVector[i] << j >> BITS_IN_WORD_MINUS_1 != 1) {
						numOfErrorPosition[i]++;
					}
				}
			}
			numOfTempErrorPattern = 1;
			for (i=0; i<SeedMaxError; i++) {
				numOfTempErrorPattern *= numOfErrorPosition[i];
			}
			MMUnitFree(numOfErrorPosition, SeedMaxError * sizeof(int));

			tempErrorPosition = MMUnitAllocate((SeedMaxError + 1) * sizeof(int));
			tempErrorPattern = MMUnitAllocate(numOfTempErrorPattern * sizeof(unsigned int));
			for (i=1; i<=SeedMaxError; i++) {
				tempErrorPosition[i] = SeedLength + 1;
			}
			errorIndex = 1;
			tempErrorPatternGenerated = 0;
			while (errorIndex > 0) {
				if (tempErrorPosition[errorIndex] > 0) {
					// Try to move the error to the left
					tempErrorPosition[errorIndex]--;
					while (tempErrorPosition[errorIndex] > 0) {
						if (MatchBitVector[i] << (tempErrorPosition[errorIndex]-1) >> BITS_IN_WORD_MINUS_1 != 1) {
							break;
						}
						tempErrorPosition[errorIndex]--;
					}
					if (errorIndex >= SeedMaxError) {
						// Output an error pattern
						errorPattern = 0;
						for (i=1; i<=SeedMaxError; i++) {
							if (tempErrorPosition[i] > 0) {
								errorPattern |= (FIRST_BIT_MASK >> (SeedLength - tempErrorPosition[i]));
							}
						}
						tempErrorPattern[tempErrorPatternGenerated] = errorPattern;
						tempErrorPatternGenerated++;
					} else {
						// Switch to the next error
						errorIndex++;
					}
				} else {
					// All error positions for this error has been emulated
					tempErrorPosition[errorIndex] = SeedLength + 1;
					errorIndex--;
					continue;
				}
			}
			if (tempErrorPatternGenerated != numOfTempErrorPattern) {
				printf("Error pattern generation error!\n");
				exit(1);
			}
			QSort(tempErrorPattern, numOfTempErrorPattern, sizeof(unsigned int), TempErrorPatternOrder);
			tempErrorPatternGenerated = 0;
			for (i=0; i<numOfTempErrorPattern; i++) {
				if (tempErrorPattern[tempErrorPatternGenerated] != tempErrorPattern[i]) {
					tempErrorPatternGenerated++;
					tempErrorPattern[tempErrorPatternGenerated] = tempErrorPattern[i];
				}
			}
			tempErrorPatternGenerated++;

			hitCombination.numOfCombination = tempErrorPatternGenerated;
			hitCombination.maxError = SeedMaxError;
			hitCombination.keyLength = SeedLength;
			hitCombination.errorPos = MMUnitAllocate(tempErrorPatternGenerated * SeedMaxError * sizeof(int*));

			// Convert error patterns to error positions
			for (i=0; i<tempErrorPatternGenerated; i++) {
				for (j=0; j<hitCombination.maxError; j++) {
					hitCombination.errorPos[i * hitCombination.maxError + j] = 0;
				}
				errorIndex = 0;
				errorPattern = tempErrorPattern[i];
				for (j=0; j<SeedLength; j++) {
					if ((errorPattern & FIRST_BIT_MASK) != 0) {
						hitCombination.errorPos[i * hitCombination.maxError + errorIndex] = hitCombination.keyLength - j;
						errorIndex++;
			...
			if (hitCombination.keyLength >= saIndexRangeNumOfChar) {
				hitCombination.skipTableWidth = hitCombination.keyLength - saIndexRangeNumOfChar;
			} else {
				hitCombination.skipTableWidth = 0;
			}
			if (hitCombination.skipTableWidth > 0) {
				hitCombination.skip = MMUnitAllocate(tempErrorPatternGenerated * hitCombination.skipTableWidth * sizeof(int*));
				hitCombination.skipErrorIndex = MMUnitAllocate(tempErrorPatternGenerated * hitCombination.skipTableWidth * sizeof(int*));
			} else {
				hitCombination.skip = NULL;
				hitCombination.skipErrorIndex = NULL;
			}

			
			// Find the skips when pattern not found or all patterns retrieved
			for (i=0; i<tempErrorPatternGenerated; i++) {
				k = i+1;
				mask = ~(ALL_ONE_MASK >> hitCombination.keyLength);
				for (j=0; j<hitCombination.skipTableWidth; j++) {
					errorIndex = 0;
					while (errorIndex < hitCombination.maxError && hitCombination.errorPos[i * hitCombination.maxError + errorIndex] > j) {
						errorIndex++;
					}
					errorPattern = tempErrorPattern[i] & mask;
					while (k<tempErrorPatternGenerated && (tempErrorPattern[k] & mask) == errorPattern) {
						k++;
					}
					if (k < tempErrorPatternGenerated) {
						errorIndex = 0;
						while (hitCombination.errorPos[k * hitCombination.maxError + errorIndex] == hitCombination.errorPos[i * hitCombination.maxError + errorIndex]) {
							errorIndex++;
						}
						hitCombination.skipErrorIndex[i*hitCombination.skipTableWidth + j] = errorIndex;
					} else {
						hitCombination.skipErrorIndex[i*hitCombination.skipTableWidth + j] = 0;
					}
					hitCombination.skip[i*hitCombination.skipTableWidth + j] = k;
					mask <<= 1;
				}
			}


			MMUnitFree(tempErrorPattern, numOfTempErrorPattern * sizeof(unsigned int));
			MMUnitFree(tempErrorPosition, (SeedMaxError + 1) * sizeof(int));

		} else {
			hitCombination.numOfCombination = 1;
			hitCombination.maxError = 0;
			hitCombination.keyLength = SeedLength;
			hitCombination.errorPos = NULL;
		}
*/
		...
		while (!feof(queryFile)) {
			...
			while (!feof(queryFile) && queryPatternLength < BWTBLAST_MAX_QUERY_LENGTH && queryChar != '>') {
				// Get query pattern
				if (queryChar != '\n') {
			...
			// Allocate memory for forms of query input
			packedQueryPattern = MMUnitAllocate(((queryPatternLength + 16 - 1) / 16 + 1) * sizeof(int));
			packedMask = MMUnitAllocate(((queryPatternLength + 16 - 1) / 16 + 1) * sizeof(int));


			// Handling masking

			if (!MaskLowerCase) {
				// Change lower case in query input to upper case if not masking lowercase
			...
			if (!SoftMasking) {
				...
				// Mask query pattern for hard masking
				...
				// Mask query pattern for soft masking

				hitQueryPattern = MMUnitAllocate(queryPatternLength);	// only allocated for soft masking
				memcpy(hitQueryPattern, queryPattern, queryPatternLength);
				...
				for (i=0; i<queryPatternLength; i++) {
					if (hitQueryPattern[i] >= 'a' && hitQueryPattern[i] <= 'z') {
						hitQueryPattern[i] = lowercaseDnaCharIndex;
					} else {
						hitQueryPattern[i] = charMap[queryPattern[i]];
					}
				}
			...
			numOfSubPattern = (queryPatternLength - SeedLength + SeedSkip + 1) / (SeedSkip + 1);
			numOfHitForQuery = 0;
			...
			for (currentContext=0; currentContext<numOfContext; currentContext++) {
				...
				if (contextInfo[context[currentContext].contextNum].reversed && contextInfo[context[currentContext].contextNum].complemented) {
					...
					if (SoftMasking) {
						for (i=0; i<queryPatternLength/2; i++) {
							c = charMap[complementMap[dnaChar[hitQueryPattern[i]]]];
							hitQueryPattern[i] = charMap[complementMap[dnaChar[hitQueryPattern[queryPatternLength - 1 - i]]]];
							hitQueryPattern[queryPatternLength - 1 - i] = c;
						}
						if (queryPatternLength % 2 == 1) {
							hitQueryPattern[queryPatternLength/2] = charMap[complementMap[dnaChar[hitQueryPattern[queryPatternLength/2]]]];
						}
					}
				}

				// Prepare packed pattern and packed mask
				for (i=0; i<((queryPatternLength + 16 - 1) / 16 + 1); i++) {
					packedQueryPattern[i] = 0;
					packedMask[i] = 0;
				}
				for (i=0; i<queryPatternLength; i++) {
					if (queryPattern[i] < ALPHABET_SIZE) {
						packedQueryPattern[i/CHAR_PER_WORD] |= queryPattern[i] << (BITS_IN_WORD - (i % CHAR_PER_WORD + 1) * BIT_PER_CHAR);
					} else {
						packedMask[i/CHAR_PER_WORD] |= 0x3 << (BITS_IN_WORD - (i % CHAR_PER_WORD + 1) * BIT_PER_CHAR);
					}
				}
				...
				saIndexGroup = (SaIndexGroupNew*)(workingMemory);
				maxNumberOfSaIndexGroup = (WorkingMemorySize - workingMemoryUsed) / SAINDEXGROUP_MEMORY_FACTOR / sizeof(SaIndexGroupNew);

				hitList = NULL;
				context[currentContext].numOfHit = 0;

				queryLengthProcessed = 0;
				while (queryPatternLength - queryLengthProcessed >= SeedLength) {

					if (!SoftMasking) {
						numOfSaIndexGroup = BWTSubPatternHammingDistSaIndexOld(queryPattern, queryPatternLength, SeedLength, bwt, 
																				SeedMaxError, SeedSkip, queryLengthProcessed, &queryLengthInCurrentRound,
																				saIndexGroup, maxNumberOfSaIndexGroup);
					} else {
						numOfSaIndexGroup = BWTSubPatternHammingDistSaIndexOld(hitQueryPattern, queryPatternLength, SeedLength, bwt, 
																				SeedMaxError, SeedSkip, queryLengthProcessed, &queryLengthInCurrentRound,
																				saIndexGroup, maxNumberOfSaIndexGroup);
					}
					if (queryLengthInCurrentRound == 0) {
						fprintf(stderr, "Not enough memory allocated for SA index group!\n");
						exit(1);
					}
		...
		//																saIndexGroup, (WorkingMemorySize - workingMemoryUsed) / sizeof(SaIndexGroupNew));																firstSaIndexGroupForSubPattern);
					...
					if (hitList == NULL) {
						...
						hitList = (HitListWithPosQuery*)(workingMemory + workingMemoryUsed);
						maxNumberOfSaIndexGroup = numOfSaIndexGroup;
					}

					// Retrieve hits and perform ungapped extension by batch according to the amount of memory available
					numOfSaIndexGroupProcessed = 0;
					while (numOfSaIndexGroupProcessed < numOfSaIndexGroup) {

						hitInCurrentRound = BWTDPHit(bwt, saIndexGroup, numOfSaIndexGroup, 
													 numOfSaIndexGroupProcessed, &saIndexGroupInCurrentRound,
													 TRUE, workingMemory + workingMemoryUsed, WorkingMemorySize - workingMemoryUsed, 
													 &bwtSaRetrievalStatistics);
						if (saIndexGroupInCurrentRound == 0) {
							fprintf(stderr, "Not enough memory allocated!\n");
							exit(1);
						}
						
						totalHitGenerated += hitInCurrentRound;
						...
						hitInCurrentRound = HSPUngappedExtension(hsp->packedDNA, packedQueryPattern, packedMask, 
															queryPatternLength, SeedLength, hsp->dnaLength,
															hitList + context[currentContext].numOfHit, hitInCurrentRound,
															ungappedExtLookupTable, (int) calcUngapCutoffScore(), (int)getUngapXdropoff());

						totalUngappedHit += hitInCurrentRound;

						hitInCurrentRound = HSPRemoveDuplicateUngappedHit(hitList + context[currentContext].numOfHit, hitInCurrentRound);
						totalUniqueUngappedHit += hitInCurrentRound;

						if (PrintPerformanceStatistics) {
							elapsedTime = getElapsedTime(startTime) - totalQueryTime;
							totalQueryTime += elapsedTime;
							totalUngappedExtensionTime += elapsedTime;
						}

						context[currentContext].numOfHit += hitInCurrentRound;
						numOfSaIndexGroupProcessed += saIndexGroupInCurrentRound;
						workingMemoryUsed += hitInCurrentRound * sizeof(HitListWithPosQuery);
					}

					queryLengthProcessed += queryLengthInCurrentRound;
				...
				if (context[currentContext].numOfHit > 0) {
					context[currentContext].gappedHitList = MMUnitAllocate(context[currentContext].numOfHit * sizeof(GappedHitList));
					memcpy((char*)context[currentContext].gappedHitList
								  + context[currentContext].numOfHit * sizeof(GappedHitList) 
								  - context[currentContext].numOfHit * sizeof(HitList), 
								  hitList, context[currentContext].numOfHit * sizeof(HitList));
					hitList = (HitListWithPosQuery*)((char*)context[currentContext].gappedHitList 
															+ context[currentContext].numOfHit * sizeof(GappedHitList) 
															- context[currentContext].numOfHit * sizeof(HitList));
				}

				// gapped extension
				numOfGappedHit = HSPGappedExtension(hsp->packedDNA, hsp->dnaLength, queryPattern, queryPatternLength, 
													hitList, context[currentContext].numOfHit, 
													(GappedHitList*)(context[currentContext].gappedHitList),
													workingMemoryPool,
													stat_scoreBlock.match, stat_scoreBlock.mismatch,
													-stat_scoreBlock.gapOpen, -stat_scoreBlock.gapExt,
													calcGapCutoffScore(), getGapXdropoff());
				totalGappedHit += numOfGappedHit;

				numOfNonCrossBoundaryGappedHit = HSPSplitCrossBoundaryGappedHit((GappedHitList*)(context[currentContext].gappedHitList), numOfGappedHit, queryPatternLength, 
																			  hsp->seqOffset, stat_scoreBlock.match, calcGapCutoffScore());
				totalNonCrossBoundaryGappedHit += numOfNonCrossBoundaryGappedHit;

				numOfUniqueGappedHit = HSPRemoveDuplicateGappedHit((GappedHitList*)(context[currentContext].gappedHitList), numOfNonCrossBoundaryGappedHit);
				totalUniqueGappedHit += numOfUniqueGappedHit;

				if (PrintPerformanceStatistics) {
					elapsedTime = getElapsedTime(startTime) - totalQueryTime;
					totalQueryTime += elapsedTime;
					totalGappedExtensionTime1 += elapsedTime;
				}

				// gapped extension with traceback
				numOfGappedHitWithTraceback = HSPGappedExtensionWithTraceback(hsp->packedDNA, queryPattern, queryPatternLength, 
																			 (GappedHitList*)context[currentContext].gappedHitList, numOfUniqueGappedHit,
																			 workingMemoryPool, alignmentPool,
																			 hsp->seqOffset, hsp->ambiguity,
																			 stat_scoreBlock.match, stat_scoreBlock.mismatch,
																			 -stat_scoreBlock.gapOpen, -stat_scoreBlock.gapExt,
																			 stat_expectationValue, getGapXdropoffFinal());
			...
			if (numOfHitForQuery > 0) {
				...
				for (currentContext=0; currentContext<numOfContext; currentContext++) {
					if (context[currentContext].numOfFinalHit > 0) {
						...
						MMUnitFree(context[currentContext].gappedHitList, context[currentContext].numOfHit * sizeof(GappedHitListWithAlignment));
			...
			// Free memory for forms of query input
			if (SoftMasking) {
				MMUnitFree(hitQueryPattern, queryPatternLength);
			}
			MMUnitFree(packedQueryPattern, ((queryPatternLength + 16 - 1) / 16 + 1) * sizeof(int));
			MMUnitFree(packedMask, ((queryPatternLength + 16 - 1) / 16 + 1) * sizeof(int));
			...
			if (timingFile != NULL) {
				fprintf(timingFile, "%s, %s, %9.4f, %d\n", "BWT-BLAST", queryPatternName, totalQueryTime - queryStartTime, numUnmaskedChar);
			}
		...
		Socketprintf("\nFinished query of %d sequences. Total no. of unmasked characters = %lld.\n\n", numOfQueryPattern, totalNumOfUnmaskedChar);
		if (PrintPerformanceStatistics) {
			...
			Socketprintf("   Gapped hit     = %9lld (%lld)\n", totalUniqueGappedHit, totalGappedHit);
			Socketprintf("Gapped extension time 2 = %9.4f s", totalGappedExtensionTime2);
...
/*
		if (SeedMaxError > 0) {
			MMUnitFree(hitCombination.errorPos, tempErrorPatternGenerated * SeedMaxError * sizeof(int*));
			if (hitCombination.skipTableWidth > 0) {
				MMUnitFree(hitCombination.skip, tempErrorPatternGenerated * hitCombination.skipTableWidth * sizeof(int*));
			}
		}
*/
		if (LoadIndex) {
	...
	HSPFreeUngappedExtLookupTable(mmPool, ungappedExtLookupTable);
...
dictionary *ParseInput(int argc, char** argv) {
	...
	char t4[3] = "-U";	// specify that this is a boolean type parameter; no following argument
	char *d[4];
...
void ParseIniFile(char *iniFileName) {
	...
	AlignmentMemorySize = iniparser_getuint(ini, "Memory:AlignmentMemorySize", AlignmentMemorySize);
...
void PrintShortDesc() {
	...
	printf("BWT-BLAST v1.0, Copyright (C) 2006, Wong Chi Kwong.\n");
	printf("BWT-BLAST comes with ABSOLUTELY NO WARRENTY.\n");
	printf("BWT-BLAST is free software, and you are welcome to\n");
...
void PrintHelp() {
	...
	printf("Syntax: bwtblast <database> <query> <output>\n");
	printf("                 [-c Confirm]\n");
	printf("                 [-L Load server process]\n");
	printf("                 [-X Unload server process]\n");
	printf("                 [-U Mask lower case]\n");
	printf("                  -F T/F Mask query by Dust; m - Soft masking\n");
	printf("                  -S <strand> : 1 - Upper; 2 - Lower; 3 - Both (default)\n");
	printf("                  -e <min E-Value> (default in parameter file)\n");
...
void ParseQueryParameterFile(char *queryParameterFileName) {
	...
	int i, error;
	char MatchBitVectorText[30] = "HitGeneration:MatchBitVector1";
	char TempMatchBitVector[MAX_ARPROX_MATCH_LENGTH+1];
	...
	// Hit generation parameters
	SeedMaxError = iniparser_getint(queryParameter, "HitGeneration:SeedMaxError", SeedMaxError);
	SeedLength = iniparser_getint(queryParameter, "HitGeneration:SeedLength", SeedLength);
	SeedSkip = iniparser_getint(queryParameter, "HitGeneration:SeedSkip", SeedSkip);
	if (SeedMaxError > MAX_APPROX_MATCH_ERROR) {
		fprintf(stderr, "Hit max error is at most %d!\n", MAX_APPROX_MATCH_ERROR);
		exit(1);
	}
	for (error=0; error<SeedMaxError; error++) {
		iniparser_copystring(queryParameter, MatchBitVectorText, TempMatchBitVector, "", MAX_ARPROX_MATCH_LENGTH);
		MatchBitVector[error] = 0;
		for (i=0; i<MAX_ARPROX_MATCH_LENGTH; i++) {
			if (TempMatchBitVector[i] == '1') {
				MatchBitVector[error] |= FIRST_BIT_MASK >> i;
			}
		}
		MatchBitVectorText[29]++;
	}
	...
	// Hit Extension parameters
	stat_XdropoffsInBits.ungapXdropoff = (float)iniparser_getdouble(queryParameter,"HitExtension:UngapXdropoffInBits", stat_XdropoffsInBits.ungapXdropoff);
	stat_XdropoffsInBits.gapXdropoff = (float)iniparser_getdouble(queryParameter,"HitExtension:GapXdropoffInBits", stat_XdropoffsInBits.gapXdropoff);
	stat_XdropoffsInBits.gapXdropoffFinal =(float) iniparser_getdouble(queryParameter,"HitExtension:GapXdropoffFinalInBits", stat_XdropoffsInBits.gapXdropoffFinal);
...
void ValidateQueryParameters() {
	...
	if (SeedLength < 8) {
		fprintf(stderr, "Hit length must be at least 8!\n");
		exit(1);
	}
	if (SeedLength > MAX_ARPROX_MATCH_LENGTH) {
		fprintf(stderr, "Hit length is at most %d!\n", MAX_ARPROX_MATCH_LENGTH);
		exit(1);
	}
...
//	int i;
	...
	printf("Hit Max Error      :   %u\n", SeedMaxError);
	printf("Hit Length         :   %u\n", SeedLength);
	if (SeedSkip > 0) {
		printf("Hit Skip           :   %u\n", SeedSkip);
	}
	//for (i=0; i<SeedMaxError; i++) {
	//	printf("Match Bit Vector   :   %s\n", MatchBitVector[i]);
	//}
	...
	printf("Ungap Xdropoff(Bits)     :   %5.2f\n", stat_XdropoffsInBits.ungapXdropoff);
	printf("Gap Xdropoff(Bits)       :   %5.2f\n", stat_XdropoffsInBits.gapXdropoff);
	printf("Gap Xdropoff Final(Bits) :   %5.2f\n", stat_XdropoffsInBits.gapXdropoffFinal);
...
int TempErrorPatternOrder(const void *tempErrorPattern, const int index1, const int index2) {
	...
	if (*((unsigned int*)tempErrorPattern + index1) != *((unsigned int*)tempErrorPattern + index2)) {
		if (*((unsigned int*)tempErrorPattern + index1) > *((unsigned int*)tempErrorPattern + index2)) {
			return 1;
		} else {
			return -1;
