...
while ($queryName = <QUERY>) {
  if (substr($queryName, 0, 1) eq ">") {
    ...
    $command = $compare . " " . $compareEvalueParameter . " " . $arg . " " . $tempblastResult . " " . $tempbwtswResult . " " . "\"" . $queryName . "\"" . " " . $tempbwtswMissed . " " . $tempblastMissed . " >> " . $ARGV[2] . $evalueByQuery;
#    print $command . "\n";
    $rc = system($command);
    if ($rc != 0) {
    ...
    $command = $compare . " " . $compareRankParameter . " " . $arg . " " . $tempblastResult . " " . $tempbwtswResult . " " . "\"" . $queryName . "\"" . " " . $tempbwtswMissed . " " . $tempblastMissed . " >> " . $ARGV[2] . $rankByQuery;
#    print $command . "\n";
    $rc = system($command);
    if ($rc != 0) {
      die("blastcompare failed. Script terminated.");
...
# Summarize
$command = $compare . " -s " . $compareEvalueParameter . " " . $arg . " < " . $ARGV[2] . $evalueByQuery . " > " . $ARGV[2] . $evalueSummary;
#print $command . "\n";
$rc = system($command);
if ($rc != 0) {
    die("blastcompare failed. Script terminated.");
...
$command = $compare . " -s " . $compareRankParameter . " " . $arg . " < " . $ARGV[2] . $rankByQuery . " > " . $ARGV[2] . $rankSummary;
#print $command . "\n";
$rc = system($command);
if ($rc != 0) {
    die("blastcompare failed. Script terminated.");
